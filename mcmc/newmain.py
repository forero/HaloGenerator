...
# It defines NFW profile for density and mass
...
histo =numpy.histogram(r_walk,10000)
norm = NFW_mass(R_max)/(mass_element*n_iterations)
particles = [int(norm*p) for p in histo[0]]
r_values = numpy.delete(histo[1],1,0)

pylab.plot(r_values,particles,'')
...
for i in range(len(particles)):
	...
	for j in range(int(particles[i])):
